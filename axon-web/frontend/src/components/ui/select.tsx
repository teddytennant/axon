...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(
  ({ className, children, ...props }, ref) => (
    <select
      ref={ref}
      className={clsx(
        "h-8 w-full appearance-none rounded border border-[#1c1c1c] bg-[#0c0c0c] px-3 pr-8 text-sm text-white",
        "transition-colors cursor-pointer",
        "hover:border-[#2a2a2a]",
        "focus:border-[#2a2a2a] focus:outline-none focus:ring-1 focus:ring-white/10",
